...
def recommend(movie):
    ...
    try:
        ...
        return [{"title": "Error fetching recommendations", "poster": "", "rating": "", "release_date": "", "plot": "",
                 "director": "", "cast": []}]
...
if st.button("🚀 Let’s Go"):
    ...
    for movie in recommendations:
        with st.expander(f"📽️ {movie['title']} (More Info)"):
            st.markdown(f"<span style='color: #FFD700; font-size:18px; font-weight:bold;'>{movie['title']}</span>",
                        unsafe_allow_html=True)
            ...
            st.markdown(f"<div class='movie-info'>🎭 <b>Cast:</b> {', '.join(movie['cast'])}</div>",
                        unsafe_allow_html=True)
...
# State management for expanders
...
if st.button("🚀 Let’s Go"):
    ...
    # Store the selected movie to control which expander is open
    st.session_state.expanded_movie = None  

    for i, movie in enumerate(recommendations):
        # If this movie is selected, expand it; otherwise, keep it collapsed
